...
@tool
async def score_lead(lead_id: str, brand_id: int) -> str:
    """
    ...
    try:
        lead = await client.get_lead(brand_id, lead_id)
        engagement = await client.get_lead_engagement(brand_id, lead_id)
...
@tool
async def suggest_follow_up(lead_id: str, brand_id: int) -> str:
    """
    ...
    try:
        lead = await client.get_lead(brand_id, lead_id)
        message = await client.generate_follow_up(brand_id, lead_id)
        ...
        logger.error(f"Failed to suggest follow-up: {e}")
        return json.dumps({"success": False, "error": str(e)})