...
class Executor:
    ...
    async def _route_action(self, name: str, payload: Dict, brand_id: int) -> Any:
        ...
        if not func:
            ...
            return await self.client.create_pending_action(brand_id, {"name": name, **payload}, "Autonomous execution")
        
        # ✅ Add await to every call
        if name == "trigger_content_generation":
         return await func(brand_id, payload.get("topic", "Untitled"), payload.get("template", "blog"))
        elif name == "notify_lead_response":
             return await func(brand_id, payload.get("lead_id"))
        elif name == "pause_campaign":
            return await func(brand_id, payload.get("campaign_id"), payload.get("reason", "Paused by AI"))
        else:
